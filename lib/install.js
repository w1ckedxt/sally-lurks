...
async function install() {
  ...
  if (installable.length === 0) {
    console.log('');
    sally("I can't find anything to haunt. Install some dev tools first.");
    process.exit(1);
  }
  ...
  if (count > 0) {
    ...
    console.log('');
    info(`Run ${dim('sally-lurks status')} to see what she's watching.`);
    info(`Run ${dim('sally-lurks remove')} if you can't handle the truth.`);
  } else {
